...
- **Paper 1 — the founding vision** (`papers/01-vision/`) is **frozen** and stands alone as a self-contained vision paper (dated *June 2026*, before the executable demonstrator `poc/` existed; it does **not** reference Paper 2). It was editorially revised **once** for its arXiv preprint (July 2026 — typos, phrasing, a standalone preprint note, and removal of one proof-of-concept-preview passage; **no prediction or argument changed**; recorded in `ERRATA.md`) and then **re-frozen at the published-preprint commit**, which is where `scripts/check-freeze.py`'s `FREEZE_REF` now points. It carries its own pinned graph/diagram inputs and is errata-only again — do not edit it; the freeze guard fails the build if it drifts. A further one-time editorial-and-metadata revision for publication is permitted by the `paper-corpus` spec, but only that: no substantive claim may move.
