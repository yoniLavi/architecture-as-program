...
Paper 1 (papers/01-vision/) reproduces the repository's state at the freeze
commit — the last state before the executable demonstrator (poc/) existed. Its
value is that it is a stable, dated, historical record; that only holds if its
sources are not quietly rewritten. This check compares each frozen file against
its content at the freeze commit and fails if any has drifted.

Corrections to a frozen paper are recorded in its ERRATA.md, never by editing
the frozen text — so any diff here is a mistake, and the build should fail.
...
# The freeze point: the commit *before* poc/ was introduced (59898cc).
...
FROZEN_FILES = {
    ...
    "papers/01-vision/diagrams/typed-wiring.typ": "diagrams/typed-wiring.typ",
}
...
def main() -> int:
    ...
    probe = subprocess.run(
        ["git", "cat-file", "-e", f"{FREEZE_REF}:proposal.typ"],
        cwd=root,
        capture_output=True,
    )
