...
In production, this structural temporality would double as observability infrastructure. Every crossing of a capability boundary (every database read, network call, or LLM invocation) is a typed, observable event. A structured log of these events would constitute a record of the system's inputs (subject to the fidelity limitations discussed in Technical Note A). Given that log and a deterministic signal graph, the system's behaviour at any past point would be substantially reproducible. Debugging a production failure would mean replaying the event log in the development environment, reconstructing the exact timeline, and forking the failure point. The replay of the production event log _is_ the regression test; authoring it is not a separate step.
...
#figure(
  ...
  caption: [The CustomerSupport signal graph. The red zone marks untrusted data flow; the green zone marks the sanitised region. LLM capabilities (blue) show graduated access: the moderation LLM has no tools; the response LLM has only a lookup tool. Error edges (red) show typed failure routing. Grey annotations show other injected capabilities.],
...
The following section (@sec:why-now) explains why this synthesis is timely. The research agenda is organised in three phases of increasing scope, reflecting a realistic dependency ordering. Phase 1 produces a working demonstrator; Phase 2 hardens it for meaningful deployment; Phase 3 addresses the deeper formal and hardware integration questions.
