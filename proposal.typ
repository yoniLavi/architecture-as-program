...
This proposal argues that these two trends (agents as primary code authors, and structured intent capture as primary human activity) converge on a new software development paradigm. In this paradigm, a _signal graph_ (a program written in a functional reactive style with explicitly typed capability boundaries) is simultaneously the architecture model and the source of truth. Code is a compiled artifact derived from it. Security would be a structural property of the graph, not an aspiration enforced by review.
...
*Trust annotations* propagate through the graph. Data entering from untrusted sources carries a type marker, `Untrusted<T>`, that is preserved through transformations unless explicitly discharged by a sanitisation node. In a well-typed realisation, the type system would prevent `Untrusted<T>` from reaching a node that accepts only `T` without sanitisation. Prompt injection (the dangerous combination of `Untrusted<T>` and `LLMClient<tools>` in the same node) would be ill-typed: no well-typed wiring would connect an untrusted source directly to an LLM-capable node.
...
*Prompt injection.* The dangerous combination of untrusted input and a tool-capable LLM is ill-typed for the structural reasons described in @sec:frp-brief and illustrated in @sec:concrete-graph: no well-typed wiring would connect an untrusted source to an LLM-capable node without explicit sanitisation.
...
*Economic pressure.* Organisations adopting AI coding agents are discovering that unconstrained agents produce architectural drift and security vulnerabilities at unprecedented speed. The emergence of spec-driven development frameworks (OpenSpec @fission_openspec_2025, Kiro @aws_kiro_2025, Spec Kit @github_spec_kit_2025) is itself evidence: these tools exist because agents without structured constraints produce results that outstrip review capacity. The pattern is visible in studies reporting declining code quality metrics coincident with AI assistant adoption @gitclear_ai_2024. The need for formal architectural constraints on agent-generated code is acute and growing.

If these trends continue, there is a plausible path toward model-driven, capability-restricted, agent-implemented systems — and a choice between deliberate design and haphazard emergence from the collision of existing tools.
