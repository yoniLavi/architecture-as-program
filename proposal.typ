...
The second is _trust tainting_. Data entering the graph from untrusted sources (user input, third-party API responses, LLM outputs) carries a type marker that propagates through signal transformations until it passes through an explicitly designated sanitisation node. This is analogous to taint tracking as studied in information-flow security @sabelfeld_language-based_2003, and specifically to the labelled-IO approach demonstrated by practical information-flow control libraries such as LIO @stefan_flexible_2012, but expressed as ordinary type-level propagation rather than a separate analysis. A node that accepts `Untrusted<string>` and a node that accepts `LLMClient<tools>` cannot be directly wired; the type system would prevent the combination. The graph topology would enforce the security property without separate analysis.
...
More broadly, the signal graph's model of isolated components communicating by typed messages is an instance of the actor model @hewitt_universal_1973, and the resemblance to BEAM processes is not coincidental. However, the actor model as typically realised (Erlang, Akka, Orleans) leaves two things implicit: the set of messages an actor may _receive_ is visible in its interface, but the set of external resources it may _access_ is not, and the wiring between actors is determined at runtime by message sends rather than declared statically. Pony @clebsch_deny_2015 goes further, integrating _reference capabilities_ (iso, val, ref, box, trn, tag) directly into the actor-model type system to enforce data-race freedom at compile time. This is the closest existing integration of actor isolation and capability-based type checking. However, Pony's capabilities govern memory access patterns (aliasing, mutability), not external authority (database access, network calls, LLM invocation); the signal graph's capability model operates at the architectural level rather than the memory-reference level.
...
#figure(
  ...
  caption: [The SupportPlatform composition graph. Each service node is a sub-graph (the nine-node CustomerSupport graph is now a single node). Capability annotations show the authority distribution at the platform level. The audit node collects outcomes from all services with append-only database access.],
