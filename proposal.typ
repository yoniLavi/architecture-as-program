...
graph OrderProcessing {
  ...
  edge ProcessPayment.out     → ConfirmOrder.payment
}
```

This is simultaneously the architecture diagram, the security policy, and the program. The capability distribution is visible at a glance: only `ValidateOrder` can read the product catalogue; only `ProcessPayment` touches the payment gateway; only `ConfirmOrder` can write orders and emit events. `ParseRequest` has no capabilities at all — it is pure parsing. A reviewer examining this graph sees the entire attack surface in the wiring. An AI agent implementing `ParseRequest` receives its signature and contracts but has no mechanism to access a database or payment gateway, regardless of what code it generates.
...
*Injection attacks.* SQL injection and command injection depend on untrusted input reaching an interpreter in executable form. In the signal graph, a SQL-executing capability would accept typed queries, not raw strings. `Untrusted<string>` could not reach it without passing through a sanitisation node that produces a typed query. In a sound realisation of the type system, the pattern would be inexpressible, not merely discouraged.

*Prompt injection.* The dangerous combination (untrusted input reaching an LLM that has access to powerful tools) requires a node that accepts both `Untrusted<string>` and `LLMClient<tools>`. In a well-typed graph, no such node could appear without explicit sanitisation between the untrusted source and the LLM capability. The graph topology would make this visible as a structural property; static analysis would confirm it without inspecting implementations.
