...
- [ ] 1.1 Add an optional per-node `capability_identities` field (map: declared capability type → identity
      label) to `graphs/schema.json`, with a description matching the `inputs` capability convention
- [ ] 1.2 Decide precedence between graph-declared identity and the `assemble(identities=...)` argument
      (override vs additive vs error) and record it in design.md

## 2. Validation
- [ ] 2.1 Extend `scripts/graph_validator.py` to check identity declarations: an identity may be declared
      only for a capability the node holds; a label may be shared or distinct
- [ ] 2.2 Reject at validation time an identity for an unheld capability, mirroring the runtime's
      assembly-time rejection (a test asserts both agree)

## 3. Assembly consumption
- [ ] 3.1 Make `assemble(...)` derive `identities` from the graph JSON; keep the `identities=` argument
      working per the chosen precedence
- [ ] 3.2 Verify revocable/rotatable targeting still works when the identity it targets is graph-declared

## 4. Rendering (no drift)
- [ ] 4.1 Surface identity in `scripts/generate-graph.py` pseudocode and diagram output so a distinct
      instance is visible in the rendered artifact
- [ ] 4.2 Optionally annotate `graphs/support-platform.json` to demonstrate distinct instances and
      sub-graph routing; regenerate figures

## 5. Sub-graph routing
- [ ] 5.1 Carry identity across the `SupportPlatform` → `CustomerSupport` boundary: the parent binds a
      named instance to the sub-graph's capability slot
- [ ] 5.2 Test that the sub-graph node receives the routed instance and not a sibling the parent did not
      route

## 6. Tests and wrap-up
- [ ] 6.1 Tests: graph-declared identity routes distinct instances; unheld-capability identity is rejected;
      no-identity graphs unchanged; sub-graph routing carries identity
- [ ] 6.2 Fold into the proposal: §4/§5 routing discussion + Technical Note A "Hierarchical capability
...
- [ ] 6.3 Full gate green: ruff, pytest, `make build` (figures regenerate from the JSON)
