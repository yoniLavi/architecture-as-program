...
#figure(
  table(
    ...
    [User-level authorisation threaded through capability injection], [Conditional], [—],
    [Behavioural contracts constrain generated implementations], [Conditional], [—],
...
User-level authorisation, on which the confused-deputy attenuation argument depends, remains undesigned. Behavioural contracts have no language and no checker; the demonstrator has tests, which is not the same thing. CHERI integration is untouched, so enforcement stops at the WASM boundary.
