...
*Language agnosticism.* WASM is the intended target (Rust, C, C++, Go, Python via interpreters such as Monty). Interfaces are defined in a language-neutral type system, so the implementation language is an optimisation choice — the basis for the interchangeability the confined tier demonstrates once (@sec:sandbox-tier).
...
*Composition is checked on the input side and unchecked on the output side.* The vision treated the union-alias convention for sub-graph outputs as the simplest of several designs and moved on; in the demonstrator it proved the cheapest to adopt and the hardest to verify — `ServiceOutcome` is asserted in the JSON and checked by nothing (@sec:subgraph). That combination is poor for the property most worth checking, that a sub-graph's boundary type honestly describes what it emits, and it argues for one of the alternative designs rather than for the convention.
...
*Sub-graph output aggregation.* The dual problem at the output side is more open than the input side, and the demonstrator sharpened rather than settled it. When a sub-graph's terminals emit distinct types, the boundary must collapse them into the single type the parent declares. The options mirror the input side: (i) define the boundary output as the union of terminal types, so `ServiceOutcome = DeliveryConfirmation | EscalationTicket` is an alias and no aggregation node is required; (ii) named output ports the parent wires individually; (iii) an explicit aggregation node inside the sub-graph. The demonstrator runs on (i), whose thinness @sec:subgraph documents — the cheapest option to adopt and the hardest to verify, its boundary type asserted and checked by nothing. The multi-terminal case, where a run reaches several terminals and there is no single boundary value, is refused by the runtime rather than resolved, and is a design question, not an implementation detail.
