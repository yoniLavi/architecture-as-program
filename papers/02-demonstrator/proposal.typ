...
One boundary is worth drawing sharply, because the trace sits next to a prediction it does not fulfil. @sec:time argues that under the runtime's purity guarantees every capability crossing would be a typed, observable event whose structured log would make past behaviour substantially reproducible — replay-and-fork debugging, a regression suite authored for free. The trace is that structured log of crossings, and having it is a genuine prerequisite for replay. But recording is not re-execution: a journal of what crossed which boundary is evidence that crossings _can_ be journalled, not a mechanism that re-runs the graph from them. Replay was not attempted (@sec:conditional), and this artifact does not move that verdict; it substantiates the narrower, adjacent fact that the crossings the vision would journal are the crossings the runtime already sees.

== The graph inspector <sec:inspector>

The repository now ships a visual *inspector* for the artifact (`ui/`): a web interface that renders the canonical graphs, triggers real execution on either enforcement tier, and overlays the resulting trace. Its design discipline is the repository's own, applied to an interface. The graph view is rendered from `graphs/*.json` served byte-for-byte by a thin execution API over the existing runtime — a contract test pins the served bytes to the files on disk — and the overlay renders the trace of @sec:trace exactly as returned, so the inspector is a pure consumer of two checked artifacts rather than a second source of truth about either. Runs execute server-side through the same assemble-and-execute path the tests and the evaluation harness exercise; the interface embeds no execution engine, and confined-tier components run under the server's wasmtime, not in the browser.

Three of its views are worth reporting because each makes a previously test-only fact inspectable. The corpus cases of @sec:eval-corpus are runnable interactively: a mutation case answers with the validator's rejection and the same pinned reason class the harness guards, displayed on the graph at the nodes the errors name — the assembly-time gate, demonstrated rather than asserted. A run's trace overlay shows per-node tiers, the trust label of the value on every traversed edge, and each node's capability crossings with their instance names, so identity routing is visible where it happens: drilling into the platform's nested `CustomerSupport` run shows the reply node's crossing recorded against the routed `customer_session` instance. And a guided prompt-injection walkthrough steps through the taint path to the sole discharge node, the tool-capable node's scoped refusal, the free-text residual — worded as @sec:eval-injection words it: attenuation, not elimination — and the host-versus-confined contrast of the same run (@fig:inspector).

Every behaviour just claimed is pinned in the discipline of @sec:evaluation: on the Python side by contract tests over the API (byte parity of the served graphs, schema-validity of returned traces, each mutation's reason class), and in the interface by an end-to-end suite covering canonical rendering, run-to-trace, taint visibility, and the tier contrast — the same suite that regenerates the committed screenshot this paper's figure uses, so the image cannot drift from the artifact. The corpus build is independent of all of it: `make build` needs no Node toolchain, and the interface's checks are opt-in targets. What the inspector is *not* is equally load-bearing, for @sec:outcomes: it views and runs graphs; it does not author them. The vision's prediction was a visual *editor* inside an agent workflow, and authoring — editing the graph from the interface with the validator in the loop — remains open (@sec:phase1).

#figure(
  image("/dist/ui/inspector-walkthrough-taint.png", width: 100%),
  caption: [The inspector mid-walkthrough, rendering a confined-tier run of the canonical graph. The `Untrusted` value's path is distinguished up to the declared discharge node and nowhere after; per-node badges report the recorded tier and trust labels; the side panel states the sole-discharge fact read from the trace itself. Generated by the end-to-end suite, not composed by hand.],
) <fig:inspector>
...
#figure(
  table(
    ...
    [Agent tooling for the graph workflow], [Not attempted], [@sec:phase1],
    [The visual graph editor], [Partial], [@sec:inspector],
  ),
...
Four claims hold under restrictions the vision did not attach to them, and stating those restrictions is more useful than the claims.
...
*The visual graph editor* exists as an *inspector*, and the difference is the restriction. The interface of @sec:inspector renders the canonical graphs, triggers real both-tier runs, and overlays the recorded traces — taint to the discharge point, capability crossings by instance, the tier contrast — which is the reviewing half of the predicted tooling, built against the trace rather than against a mock. What it does not do is author: no node is added, no edge rewired, no `with` clause granted from the interface. The workflow the vision imagined — an agent proposes a graph transformation and a human reviews it visually — therefore has its viewing side demonstrated and its editing side untouched, so the verdict is partial, not substantiated, and authoring with the validator in the loop is the recorded next step (@sec:phase1).
...
*Agent tooling and developer experience.* Build the workflow that takes a natural-language change description, proposes a graph transformation, generates node implementations, and submits them for automated verification, extending existing SDD tooling to operate on typed graph artifacts rather than prose. Alongside it, the visual graph editor and diff viewer: capability edge additions, trust boundary crossings, and sanitisation gaps must be visually salient. The inspector (@sec:inspector) narrows the viewing half of this item — rendering, running, and trace overlay exist against the real runtime — so the open editor-side question is now specifically *authoring*: graph edits made from the interface with the validator in the loop, and the diff view over them. It narrows nothing about the agent workflow, and nothing about *graph-scale comprehension*, which the demonstrator does nothing to address — the concrete example has nine nodes and real systems have hundreds. Hierarchical decomposition is the expected approach, but the interaction between hierarchical abstraction, capability wiring, and trust propagation across sub-graph boundaries has not been designed, and a type error deep in a sub-graph's wiring must produce an error message comprehensible at the level the developer is working at.
